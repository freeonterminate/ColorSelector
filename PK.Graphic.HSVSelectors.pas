...
begin
  ...
  var BaseColorRec := TAlphaColorRec(BaseColor);
...
begin
  ...
  ACanvas.Stroke.Color := BaseColor xor $00_ff_ff_ff;
...
begin
  var Hue := if (AX = 0) and (AY = 0) then 0.0 else RadToDeg(ArcTan2(AY, AX));
...
begin
  ...
  try
    ...
    Base.Map(TMapAccess.Write, Data);
    try
...
begin
  ...
  begin
    ...
    var V := EnsureRange(W0 + W1, 0, 1);
    var S := if V > 0 then EnsureRange(W0 / V, 0, 1) else 0.0;
